...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* recursiveConstruct(vector<Point<Dim>> & points, int start, int end, int curDim);

    KDTreeNode* recursiveFind(const Point<Dim> & query, KDTreeNode* root, int curDim) const;
