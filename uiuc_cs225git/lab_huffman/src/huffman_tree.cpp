...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1){
        TreeNode * tempLeft = removeSmallest(singleQueue, mergeQueue);
        TreeNode * tempRight = removeSmallest(singleQueue, mergeQueue);
        TreeNode * newMerged = new TreeNode(tempLeft->freq.getFrequency() + tempRight->freq.getFrequency());
        newMerged->left = tempLeft;
        newMerged->right = tempRight;
        mergeQueue.push(newMerged);
    }
