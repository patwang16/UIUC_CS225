...
class SquareMaze
{
  ...
  private:
    //struct for the walls of the maze. As specified in the Doxygen, only the down and right walls must be stored
    struct wall{
      wall(bool right, bool below){
        _right = right;
        _below = below;
      }
      bool _right = true;
    ...
    //private variables to store necessary parameters for each maze object
